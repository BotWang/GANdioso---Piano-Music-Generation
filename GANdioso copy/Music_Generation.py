...
def process_generated_images(image, scale, dissonance_value):
    ...
    trimmed_image = trim_durations(note_array)[0]
    notes_in_key = force_pitches(trimmed_image, scale, dissonance_value)
    notes_in_key = trim_durations(notes_in_key)[0] # trim durations again as forcing pitches creates messy durations
    processed_image = array_to_image(notes_in_key, 16, [0, 48])
    processed_image = trim_durations(image_to_array(processed_image))[1]
